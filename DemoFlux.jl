...
# ╔═╡ 1b715070-1f02-11ed-080f-5b53d243c634
begin
...
# ╔═╡ 6427c4fe-733b-4a4f-b023-8f6dbf069f5a
md"""
Classifier des chiffres écrits à la main avec Flux.jl
=====================================================
Auteur : Guillaume Cléroux, Université de Sherbrooke
"""

# ╔═╡ d528abdd-c674-4787-b6c4-fe16b02284d2
md"""
Dans ce notebook, nous allons faire une démonstration de la librairie Flux.jl

L'objectif est de vous familiariser avec du code Julia en action.
"""

# ╔═╡ 260fc3f2-b0e4-45df-9ce7-d5b20f2312a3
md"""
#### Commencons par importer les librairies nécessaires
"""

# ╔═╡ a7839381-e4eb-4f32-8c74-5b727682fc11
md"""
## Hyper-paramètres
"""

# ╔═╡ 07e852fa-280a-4d55-98cc-ad76c4fe78d1
lr::Float64 = 1e-3 			# Learning rate

# ╔═╡ 2f8a33a6-0c3e-4352-9c31-7996b784e0f6
batch_size::Int64 = 256 	# Batch_size

# ╔═╡ 94802fab-cc2a-4d5a-b8bd-0a053d1224ab
epochs::Int64 = 30 			# Nb. epochs

# ╔═╡ 92c3f6ca-4cea-4c92-8462-c09e94d4fc04
use_cuda::Bool = true 		# Use GPU if available

# ╔═╡ 08b89baa-f7d4-4d4f-8e63-f011cae7fec3
...
# ╔═╡ 11638c71-cd3b-48b2-9ef6-d2f85b6cb89f
md"""
#### Regardons un peu les données

Avec Plots.jl, nous pouvons visualiser facilement plusieurs sous graphe en un seul graphe.
"""

# ╔═╡ 3cba94f2-7810-4413-8d5d-f212f45acc84
let	
	# Loading the training data samples
	samples_x, _ = MNIST(split=:train)[1:9]
...
# ╔═╡ 5eec5b77-a068-4c22-949e-b09ac760a177
md"""
Chargement des datasets
--------------------
Maintenant qu'on a une idée générale de la forme de nos données, on peut les charger
dans un dataset afin de pouvoir faire des opérations intéressantes dessus.

###### Training dataset
train\_x représentes les images d'entrainement

train\_y représentes les étiquettes d'entrainement

###### Testing dataset
test\_x représentes les images de test

test\_y représentes les étiquettes de test
"""

# ╔═╡ 449adf64-92ea-4fd6-8584-4d924be7ab08
begin
...
# ╔═╡ 0ae97ad9-3137-48bf-8973-93305649e1bd
begin
	# Loading Dataset	
...
# ╔═╡ 50526b28-9334-4e24-aa90-6fca86c75ffe


# ╔═╡ 735108c2-03e3-4bf6-b6fa-6ee7bf0878ba
begin
...
# ╔═╡ 645f00c8-76d8-49e5-af38-69a0fb1a5ec4
md"""
## Création d'un modèle à convolution simple
"""

# ╔═╡ 4ff82f18-01ec-48b4-9286-094a7bca00e3
begin
	model = Flux.Chain(
...
# ╔═╡ a697600d-5949-4709-9aae-d9dec18d26f7
loss = logitcrossentropy

# ╔═╡ f8331542-2087-43aa-81bc-59e8e850a109
params = Flux.params(model) # model's trainable parameters

# ╔═╡ aecae869-0fb5-4ddc-97b5-f13ead877963
optimizer = ADAM(lr)

# ╔═╡ 64f9e5ef-7df3-4c37-b2bf-b0c7043f134c
losses = [] 	# List of all the losses so far

# ╔═╡ 2111ca61-b363-49ad-8a0a-f9895f9ff938
md"""
#### Entraînement de modèle


Maintenant que tout est prêt, nous pouvons entraîner notre modèle. 
"""

# ╔═╡ 09cd2c13-4492-45e1-950d-a46120a0def1
for epoch in 1:epochs
		...
		push!(losses, avg_loss / length(train_loader))
	end

# ╔═╡ d673f479-272d-4d16-97e9-78a9eca21a3a
plot(1:epochs, losses)

# ╔═╡ 877560ab-3f8c-4d6a-ad42-ea511dc6b4b8
begin
	local acc = 0.0
	local n = 0
...
# ╔═╡ 162ea3bd-3518-498c-8c09-71bbc195b056
md"""
#### Testons maintenant notre modèle

Avec Pluto, nous pouvons injecté du code JavaScript dans notre notebook et avoir des sessions intéractives.

On peut se servir de cette fonctionnalité pour s'amuser avec notre nouvel IA.
"""

# ╔═╡ 4b3b0728-762f-410f-a281-ebd8cb9b48ab
@bind drawing HTML("""
<div id=parent>
...
# ╔═╡ d1768db0-b5f4-4ef5-9f59-df03b69e2f06
...
# ╔═╡ 94b2d71b-dd6a-4d22-bed8-863cbcbca6e0
img = imrotate(Gray.(convert_drawing(drawing)[:, :, 1, 1]), -π/2)

# ╔═╡ 29cc401c-57c8-4990-a742-7d90b42428eb
begin
	# Convert the drawing into usable data for the model
	data = convert_drawing(drawing)
	...
	# Reshaping output into a list
	pred = reshape(pred, 10)
	
	bar(pred, 
		title="Trained model's prediction", 
		label=nothing,
		color = ifelse.(pred .== max(pred...), :red, :blue),
		xlims = (0, 11),
		xticks=(0:10, -1:9))
